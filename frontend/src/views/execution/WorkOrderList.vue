<script setup lang="ts">
import { onMounted, reactive, ref } from 'vue'
import TablePage from '@/components/TablePage.vue'
import { fetchWorkOrders, type WorkOrderItem } from '@/api/execution/workorder'
import { useTable } from '@/hooks/useTable'
import ActionBar from '@/components/ActionBar.vue'
import FormModal from '@/components/FormModal.vue'
import SearchBar from '@/components/SearchBar.vue'

const formFields = [
  { label: '工单号', name: 'woNo', type: 'input' as const, placeholder: '请输入工单号' },
  { label: '订单号', name: 'orderNo', type: 'input' as const, placeholder: '请输入订单号' },
  { label: '款式名称', name: 'productName', type: 'input' as const, placeholder: '请输入款式名称' },
  { label: '数量', name: 'qty', type: 'input' as const, placeholder: '请输入数量' },
  { label: '所属车间', name: 'line', type: 'input' as const, placeholder: '如 缝纫车间' },
]

const formModel = reactive({ woNo: '', orderNo: '', productName: '', qty: '', line: '' })

const rules = {
  woNo: [{ required: true, message: '请输入工单号' }],
  orderNo: [{ required: true, message: '请输入订单号' }],
  productName: [{ required: true, message: '请输入款式名称' }],
  qty: [{ required: true, message: '请输入数量' }],
  line: [{ required: true, message: '请输入所属车间' }],
}

const searchForm = reactive({ keyword: '', status: undefined as string | undefined })

const searchFields = [
  { label: '关键字', name: 'keyword', type: 'input', placeholder: '工单号/订单号', width: '200px' },
  {
    label: '状态',
    name: 'status',
    type: 'select',
    placeholder: '全部',
    options: [
      { label: '待派工', value: '待派工' },
      { label: '执行中', value: '执行中' },
      { label: '已完成', value: '已完成' },
    ],
  },
]

const columns = [
  { title: '工单号', dataIndex: 'woNo', key: 'woNo' },
  { title: '订单号', dataIndex: 'orderNo', key: 'orderNo' },
  { title: '款式名称', dataIndex: 'productName', key: 'productName' },
  { title: '数量', dataIndex: 'qty', key: 'qty' },
  { title: '车间', dataIndex: 'line', key: 'line' },
  { title: '状态', dataIndex: 'status', key: 'status' },
]

const { data, loading, pagination } = useTable<WorkOrderItem>()

const modalOpen = ref(false)

const actionBar = [
  { key: 'new', label: '新建工单', type: 'primary' },
]

const handlePageAction = (key: string) => {
  if (key === 'new') openModal()
}

const openModal = () => { modalOpen.value = true }
const handleOk = () => { modalOpen.value = false }

const loadData = async () => {
  loading.value = true
  data.value = await fetchWorkOrders({ keyword: searchForm.keyword, status: searchForm.status })
  pagination.total = data.value.length
  loading.value = false
}

const resetSearch = () => {
  searchForm.keyword = ''
  searchForm.status = undefined
  loadData()
}

onMounted(loadData)
</script>

<template>
  <TablePage title="工单管理" :columns="columns" :data="data" :loading="loading" :pagination="pagination">
    <template #search>
      <SearchBar :model="searchForm" :fields="searchFields" @search="loadData" @reset="resetSearch" />
    </template>
    <template #actions>
      <ActionBar :actions="actionBar" @action="handlePageAction" />
    </template>
  </TablePage>

  <FormModal v-model:open="modalOpen" title="新建工单" :model="formModel" :rules="rules" :fields="formFields" @ok="handleOk" />
</template>

...
不要先删历史 Python 方案后再重构主链路，正确迁移方式应为：

1. 保持前端业务接口不变
2. 保持数据库表结构不变
3. 只替换 Java 内部的质检检测实现

迁移链路如下：

原方案：

`前端 -> Java -> Python FastAPI -> YOLO`

目标方案：

`前端 -> Java -> ONNX Runtime -> YOLO ONNX模型`
...
1. 先在 Java 中完成 ONNX Runtime 本地推理跑通
2. 使用同一张测试图片验证 ONNX 推理输出可用
3. 接入 Java 业务落库链路
4. 保持前端接口不变完成联调
5. 确认链路稳定后，再废弃历史 Python 方案

说明：

- 不建议一开始就先删除 Python 方案描述再开发
- 迁移核心是“替换内部实现”，不是“推翻前端和数据库”
